...
class SudokuSolver:
    def __init__(self,puzzle_string):
        ...
        while True:
            ...
            print(f'Iteration={self.iteration}, {self.unknowns} unknowns left, {self.tot_pmarks} pencil marks left...')
            self.print_sudoku()
            self.iteration +=1
            ...
            current_unknowns = np.sum(self.puzzle==0)
            current_tot_pmarks = self.pmarks.sum()
            
            # Break conditions
            if current_tot_pmarks == self.tot_pmarks:
                # If stuck, try guess_and_check
                self.guess_and_check()
                self.expert_checks()
                if current_tot_pmarks == self.pmarks.sum():
                    print(f"Can't find solution. {self.unknowns} unknowns left, {self.tot_pmarks} pencil marks left...")
                    self.report_pmarks()
                    break
                ...
                self.unknowns = current_unknowns
                self.tot_pmarks = current_tot_pmarks
    ...
    def print_sudoku(self):
        ...
        for irow in np.arange(9):
            rowstring = ''
            for icol in np.arange(9):
                rowstring = rowstring + f'{self.puzzle[irow,icol]} '
                if icol in [2,5]:
                    rowstring = rowstring + '|'
            print(rowstring)
            if irow in [2,5]:
